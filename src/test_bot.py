...
def test_should_find_correct_neighbours():
    ...
    correct_neighbours_indices = [0, 1, 2, 13, 23, 24, 11, 12, 22, 33, 34, 35,
                                  25, 14, 3]
    answer_tiles_indices = [tile.index for tile in
                            find_neighbour_tiles(hexagons,
                                                 colored_tiles_block)]
...
def test_should_find_correct_winning_tiles_for_color():
    ...
    winning_tiles = find_winning_tiles_for_color(hexagons, empty_tiles,
                                                 winner_color)
...
def test_should_find_concrete_border_tiles():
    ...
    for index in colored_tiles_indices:
        hexagons[index].set_color(color)
    border0_tiles = [hexagon for hexagon in hexagons if
                     0 in hexagon.border_indices]
    border0_tiles = find_concrete_border_tiles(hexagons, border0_tiles,
                                               color)
    correct_border_tile_indices = [0, 11, 12, 13, 24, 34, 33, 44, 55, 56, 67,
                                   77, 88, 99, 110]
...
def test_should_find_field_border_tiles():
    ...
    correct_border1_tile_indices = [0, 11, 12, 13, 24, 34, 33, 44, 55, 56, 67,
                                    77, 88, 99, 110]
    correct_border2_tile_indices = [9, 20, 21, 32, 43, 54, 65, 76, 87, 98,
                                    109, 120]
...
def test_should_find_neighbours_of_not_opposite_color():
    ...
    answer_tiles_indices = [tile.index for tile in
                            find_neighbours_of_not_opposite_color(hexagons,
                                                                  tile,
                                                                  color)]
    for index in correct_neighbours_indices:
        assert index in answer_tiles_indices
...
def test_should_find_path_from_tile_to_tile():
    hexagons = initialize_field()
    path = find_path_from_tile_to_tile(hexagons, hexagons[0],
                                       hexagons[110])
