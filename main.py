...
def blit_text_at_centered_coordinates(
    screen,
    text: str,
...
def message_winner_to_screen(screen, color: Tuple[int]):
    ...
    if color == WHITE:
        ...
        text = "Red won!"
    blit_text_at_centered_coordinates(
        screen,
        text,
        winner_text_font,
        SCREEN_WIDTH / 2,
...
def draw_menu_screen(screen):
    ...
    button1_coordinates = find_button_coordinates(
    ...
    blit_text_at_centered_coordinates(
        ...
        buttons_y + button_height / 2,
        BLACK,
        GREEN,
    )

    button2_x = SCREEN_WIDTH / 5 * 3
    button2_coordinates = find_button_coordinates(
...
def draw_tiles():
    ...
    for i in range(FIELD_SIZE):
        ...
        screen.blit(
            text_surface,
            (hexagons[i].coordinates[0][0] - 10, hexagons[i].coordinates[0][1] + 10),
        )
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        if hovrd_hex_index != -1 and hexagons[hovrd_hex_index].color == WHITE:
            turn_color, state, game_winner = make_turn(
                hexagons, hovrd_hex_index, turn_color, state
            )
            if is_bot_on and game_winner == WHITE:
                turn_color, state, game_winner = make_turn(
                    hexagons, hovrd_hex_index, turn_color, state, is_bot=True
                )
...
def handle_menu_event(
    ...
    if event.type == pygame.QUIT:
        state = GameState.FINISHED
    if event.type == pygame.MOUSEMOTION:
        if is_inside_polygon(with_bot_btn_coordinates, pygame.mouse.get_pos()):
            hovered_menu_button_index = 1
        elif is_inside_polygon(without_bot_btn_coordinates, pygame.mouse.get_pos()):
...
if __name__ == "__main__":
    ...
    with_bot_btn_coordinates, without_bot_btn_coordinates = draw_menu_screen(screen)

    while state == GameState.MENU:
        for event in pygame.event.get():
            state, is_bot_on, hvrd_btn_indx = handle_menu_event(
                state, is_bot_on, hvrd_btn_indx, event
            )
    ...
    while state == GameState.IN_PROGRESS:
        ...
        for event in pygame.event.get():
            state, turn_color, hovered_hex_index, winner = handle_game_event(
                hexagons, is_bot_on, state, turn_color, hovered_hex_index, event
            )
        if hovered_hex_index != -1:
            pygame.draw.polygon(screen, GREEN, hexagons[hovered_hex_index].coordinates)
        pygame.display.update()
    if winner != WHITE:
        draw_tiles()
        pygame.display.update()
